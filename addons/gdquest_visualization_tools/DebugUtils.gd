...
static func array_to_texture(xs: Array) -> ImageTexture:
	...
	var format := Image.FORMAT_RF if xs[0] is int or xs[0] is float else Image.FORMAT_RGF if xs[0] is Vector2 else Image.FORMAT_RGBF
	for x in xs:
...
static func draw_polyline_dashed(canvas_item: CanvasItem, curve: Curve2D, color: Color, width := 1.0, sample := 24, sample_scale := 1.0) -> void:
	sample = int(sample * sample_scale)
	var fraction := curve.get_baked_length() / sample
	for step in range(0, sample):
		if step % 2 != 0:
			continue
		canvas_item.draw_line(curve.sample_baked(step * fraction), curve.sample_baked((step + 1) * fraction), color, width)
...
static func get_curve_capsule(radius: float, height: float, sample := 24, transform := Transform2D.IDENTITY) -> Curve2D:
