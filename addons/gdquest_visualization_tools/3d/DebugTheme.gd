...
const SHADERS := {ThemeType.WIREFRAME: preload("shaders/Wireframe.tres"), ThemeType.HALO: preload("shaders/Halo.tres")}
const THEME_FRESNEL_POWER_PROPERTY := {"name": "theme_fresnel_power", "type": TYPE_FLOAT, "hint": PROPERTY_HINT_RANGE, "hint_string": "0.01,5"}
const THEME_EDGE_INTENSITY_PROPERTY := {"name": "theme_edge_intensity", "type": TYPE_FLOAT, "hint": PROPERTY_HINT_RANGE, "hint_string": "0.0,5"}

var palette_property := {"name": "palette", "type": TYPE_INT, "hint": PROPERTY_HINT_ENUM, "hint_string": DebugUtils.enum_to_string(DebugPalette.Type)}
var theme_property := {"name": "theme", "type": TYPE_INT, "hint": PROPERTY_HINT_ENUM, "hint_string": DebugUtils.enum_to_string(ThemeType)}
...
func free_rids() -> void:
	for key in rids:
		for rid in rids[key]:
			Callable(RenderingServer, "free_rid").call(rid)
		rids[key].clear()
...
func get_property(name: String):
	match name:
		...
		"theme":
			return theme
		"theme_fresnel_power":
			return theme_fresnel_power
		"theme_edge_intensity":
			return theme_edge_intensity
