...
func _update_rayshape() -> Array:
	...
	mesh.rings = 0
	result.push_back({"primitive_type": RenderingServer.PRIMITIVE_TRIANGLES, "arrays": mesh.get_mesh_arrays()})
		...
		[_, DebugTheme.ThemeType.WIREFRAME]:
			...
			if mesh.get_surface_count() > 0:
				...
				if shape.get_class() == "SeparationRayShape3D":
					...
					meshes_info.push_back({"primitive_type": RenderingServer.PRIMITIVE_LINES, "arrays": sphere_shape.get_debug_mesh().surface_get_arrays(0)})
		[true, DebugTheme.ThemeType.HALO]:
			meshes_info = Callable(self, method_name).call()
